...
void upload_firmware() {
    ...
    write_register(Config1, 0x01);

    write_register(Lift_Config, 0x03);
	write_register(Raw_Data_Threshold, 0x00);
...
void pmw3360_frame_capture(uint8_t *buffer) {
	/* perform_startup(); */
	
	sleep_ms(50);
		
	write_register(Config2, 0x00);
	write_register(Frame_Capture, 0x83);
	write_register(Frame_Capture, 0xc5);
	sleep_ms(20);

	// Read raw frame data into buffer
	cs_select();
    uint8_t data = Raw_Data_Burst & 0x7f; // read burst destination adress
    spi_write_blocking(SPI_PORT, &data, 1);
    sleep_us(35); 

	spi_read_blocking(SPI_PORT, 0, buffer, 1296);
